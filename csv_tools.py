...
def sort_grc(file="greek.csv", column="Ref."):
    ...
    with open(file, "r", encoding="utf8") as conj_csv:
        # has to be forced into a list so that we can close the file
        conjj = csv.DictReader(conj_csv)
        # fieldnames are the first row of the csv file so don't need to be provided
        sorted_conj = sorted(conjj, key=lambda row: human_sort_dict(
            row, column))  # old key=lambda row: row[column]
    try:
        with open(file, "w", encoding="utf8", newline="") as conj_csv:
            writer = csv.DictWriter(
                conj_csv, fieldnames, lineterminator=os.linesep)
            writer.writeheader()
        ...
        # need date in case multiple errors occur
        with open(f"error{datetime.now().isoformat()}.txt", "w", encoding="utf8") as err_txt:
            err_txt.write(str(e))
...
def format_ref_column():
    ...
    with open("attested-conjectures.csv", "r", encoding="utf8") as conj_csv:
        # needs reopening before for some reason - #TODO: fix this
        conjj = csv.DictReader(conj_csv)
        new_conjj = []
        for row in conjj:
            old_ref = row["Ref."]
